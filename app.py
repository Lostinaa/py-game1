...
class DialogueSystem:
    ...
    async def update_voice_settings(self, npc_role):
        ...
        try:
            ...
            # - "fable": Male voice, deep and authoritative  
            # - "onyx": Male voice, deep and serious
    ...
    async def switch_voice_manually(self, voice_name, speed=1.0):
        ...
        try:
            ...
            print(f"[DialogueSystem] Voice manually switched to {voice_name} with speed {speed}")
            
        except Exception as e:
            print(f"[DialogueSystem] Error switching voice manually: {e}")
    ...
    def start_audio_recording(self):
        ...
        try:
            ...
            self.audio_stream = sd.InputStream(
                ...
                dtype="int16",
                blocksize=int(SAMPLE_RATE * 0.05)  # 50ms chunks (increased to reduce chunking)
            )
    ...
    def start_conversation(self, npc_role="HR", player_pos=None):
        ...
        # Update voice settings for the current NPC
    ...
    def render(self):
        ...
        if self.active:
            ...
            # Quit instruction
            quit_text_surface = self.font.render("Press Shift+Q to exit, F5 to record voice, F6 for audio status, F7 to test voices", True, (255, 255, 255))
    ...
    def handle_input(self, event):
        ...
        if event.type == pygame.KEYDOWN:
            ...
            # Test different voices with F7
            if event.key == pygame.K_F7:
                if self.current_npc == "HR":
                    asyncio.run_coroutine_threadsafe(self.switch_voice_manually("shimmer", 1.0), self.loop)
                elif self.current_npc == "CEO":
                    asyncio.run_coroutine_threadsafe(self.switch_voice_manually("fable", 0.8), self.loop)
                else:
                    asyncio.run_coroutine_threadsafe(self.switch_voice_manually("alloy", 1.0), self.loop)
